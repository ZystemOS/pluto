...
test "copyData" {
    ...
    try std.testing.expectEqual(vmm2_free_entries, vmm2.bmp.num_free_entries);
    ...
    // Test NotAllocated
    try std.testing.expectError(VmmError.NotAllocated, vmm2.copyData(&vmm, buff[0..buff.len], alloc + alloc1_blocks * BLOCK_SIZE, true));
    try std.testing.expectEqual(vmm_free_entries, vmm.bmp.num_free_entries);
    try std.testing.expectEqual(vmm2_free_entries, vmm2.bmp.num_free_entries);

    // Test Bitmap.Error.OutOfBounds
    try std.testing.expectError(bitmap.Bitmap(usize).BitmapError.OutOfBounds, vmm2.copyData(&vmm, buff[0..buff.len], vmm.end, true));
    try std.testing.expectError(bitmap.Bitmap(usize).BitmapError.OutOfBounds, vmm.copyData(&vmm2, buff[0..buff.len], vmm2.end, true));
    try std.testing.expectEqual(vmm_free_entries, vmm.bmp.num_free_entries);
    try std.testing.expectEqual(vmm2_free_entries, vmm2.bmp.num_free_entries);
}
