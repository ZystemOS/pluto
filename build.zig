...
pub fn build(b: *Builder) !void {
    ...
    const make_iso = switch (target.getCpuArch()) {
        ...
        .aarch64 => b.addSystemCommand(&[_][]const u8{ "aarch64-linux-gnu-objcopy", exec.output_path, "-O", "binary", try fs.path.join(b.allocator, &[_][]const u8{ exec.output_dir.?, "kernel8.img" }) }),
        else => unreachable,
    };
